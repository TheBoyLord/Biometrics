...
import { StyleSheet, TouchableOpacity, ActivityIndicator, View, Text, Alert } from 'react-native';
...
const showStatement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (pdfData) {
      if(pdfData.status === 'OK' && pdfData.pdfUrl) {
        const downloadPDF = async () => {
          try {
            ...
            console.error(error);
            setPdfError('Failed to download the PDF');
          } finally {
            setLoading(false);
          }
        };
        downloadPDF(); // Download the PDF using the provided URL
      } else if(pdfData.status === 'WARNING' || pdfData.status === 'ERROR') {
  ...
  const { loading: deletingPdf, error: deleteError } = useFetch<string>(
   ...
   // Show error message if PDF generation or download fails
   if (pdfError) {
    return (
